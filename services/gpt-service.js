...
class GptService extends EventEmitter {
  ...
  validateFunctionArgs(args) {
    try {
      ...
      return {}; // Return empty object if unable to parse JSON
    }
  }

  updateUserContext(name, role, text) {
    // Ensure the role is one of the allowed values or a function name
    if (!['system', 'assistant', 'user', 'function'].includes(role) && !Object.keys(availableFunctions).includes(role)) {
      console.warn(`Invalid role "${role}" provided. Using "user" role instead.`);
      role = 'user';
    }
  ...
  async completion(text, interactionCount, role = 'user', name = 'user') {
    ...
    for await (const chunk of stream) {
      ...
      if (finishReason === 'tool_calls') {
        ...
        // Update the user context with the function response using the function name as the role
